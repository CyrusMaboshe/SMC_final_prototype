...
import { studentAPI, accountantAPI, FinancialRecord, Payment, supabase } from '@/lib/supabase';
import { useStudentFinancialRealTimeUpdates } from '@/hooks/useRealTimeUpdates';

interface StudentFinancialStatementsProps {
  studentId?: string;
  userEmail?: string;
}
...
const StudentFinancialStatements: React.FC<StudentFinancialStatementsProps> = ({ studentId, userEmail }) => {
  console.log('StudentFinancialStatements component received studentId:', studentId);
  console.log('StudentFinancialStatements component received userEmail:', userEmail);
  ...
  // Set up real-time updates for financial data using student number
  useStudentFinancialRealTimeUpdates(studentId || '', () => {
    console.log('Real-time financial update triggered for student:', studentId);
    loadFinancialData();
  });
  ...
  const loadFinancialData = async () => {
    try {
      ...
      console.log('Student ID value:', JSON.stringify(studentId));
      console.log('User email:', userEmail);

      let actualStudentId = studentId;

      // If no studentId but we have userEmail, try to find student by email
      if (!studentId && userEmail) {
        console.log('No studentId provided, trying to find student by email:', userEmail);
        try {
          ...
          if (student && !error) {
            ...
            console.log('Found student by email:', actualStudentId);
          } else {
            console.error('Could not find student by email:', error);
          }
        } catch (emailError) {
      ...
      if (!actualStudentId) {
        console.error('No student ID available (neither direct nor from email)');
        setLoading(false);
        return;
      }

      const [payments, financialRecords] = await Promise.all([
        studentAPI.getPayments(actualStudentId),
        studentAPI.getFinancialRecords(actualStudentId)
      ]);
      ...
      // Convert string decimals to numbers for proper calculation
      const totalOwed = financialRecords?.reduce((sum: number, record: any) => {
      ...
      if (financialRecords && financialRecords.length > 0) {
        ...
        totalBalance = financialRecords.reduce((sum: number, record: any) => {
  ...
  const testAPIDirectly = async () => {
    ...
    try {
      console.log('Testing with hardcoded student ID: SMC2025001');
      const [payments, financialRecords] = await Promise.all([
        studentAPI.getPayments('SMC2025001'),
        studentAPI.getFinancialRecords('SMC2025001')
      ]);
      console.log('Direct API call - Payments:', payments);
      console.log('Direct API call - Financial Records:', financialRecords);
    } catch (error) {
...
export default StudentFinancialStatements;
